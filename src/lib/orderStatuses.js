...
// cache (the vocabulary is tiny and changes rarely). Callers/HTTP speak readable
// KEYS ('ready'); orders stores the compact surrogate `status_id`, so this module
// also bridges key↔id (idForKey / keyForId) at the DB boundary — keeping magic ids
// out of route code while the hot orders table stays lean. See order_status_surrogate.sql.
...
async function load() {
  const { data, error } = await supabase
    .from('order_statuses')
    .select('id, key, label, phase, sort_order, is_terminal, customer_visible, tone')
    .order('sort_order');
...
// orders stores status_id; HTTP + route code speak keys. Translate at the boundary.
export async function idForKey(key) {
  const s = (await getOrderStatuses()).find(x => x.key === key);
  if (!s) throw new Error(`Unknown order status key: ${key}`);
  return s.id;
}

export async function keyForId(id) {
  const s = (await getOrderStatuses()).find(x => x.id === id);
  return s?.key ?? null;
}

// Resolve a list of keys to ids (for `.in()` filters). Unknown keys are skipped.
export async function idsForKeys(keys) {
  const rows = await getOrderStatuses();
  return keys.map(k => rows.find(x => x.key === k)?.id).filter(id => id != null);
}
